...
class BEEFISO(BaseLearner):
    ...
    # 完成一个任务后的清理工作，更新网络状态
    def after_task(self):
        self._network_module_ptr.update_fc_after()  # 更新网络的全连接层
        self._known_classes = self._total_classes  # 更新已知的类别数量
    ...
    def incremental_train(self, data_manager):
        ...
        self.initialize_data_loaders(data_manager)
    ...
    def freeze_parameters(self):
        if self._cur_task > 0:
            ...
            for p in self._network.old_fc.parameters():
                p.requires_grad = False
        logging.info(f"All params: {count_parameters(self._network)}")
        logging.info(f"Trainable params: {count_parameters(self._network, True)}")

    def initialize_data_loaders(self, data_manager):
        ...
        if self._cur_task > 0:
            _, val_dset = data_manager.get_dataset_with_split(
                np.arange(self._known_classes, self._total_classes), source='train',
                mode='train', appendent=self._get_memory(), val_samples_per_class=int(self.samples_old_class)
            )
    ...
    def _expansion(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss_clf = F.cross_entropy(train_logits, pseudo_targets.long())
    ...
    def get_energy_loss(self,inputs,targets,pseudo_targets):
        ...
        energy_loss = F.cross_entropy(logits,targets.long())
        return energy_loss
